"""
Service request workflow (MVP).

Clients can submit a request when a service isn't currently offered.
For the final submission build:
- clients can submit requests
- providers can view requests (read-only)
- admins manage the request lifecycle (close)

Client request history (/my/requests) is intentionally disabled to match the user manual.
"""
...
def provider_requests():
    ...
    Kept temporarily so older links don't break.
    The provider blueprint owns the canonical requests page at /provider/requests.
    """
...
@login_required
@role_required("provider")
def claim_request(request_id: int):
    """Provider action disabled.

    Providers can view requests, but the request lifecycle is admin-managed.
    """
...
@login_required
@role_required("admin")
def admin_close_request(request_id: int):
